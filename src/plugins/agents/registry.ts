...
export class AgentRegistry {
  ...
  private pendingInstances: Map<string, Promise<AgentPlugin>> = new Map();
  ...
  async getInstance(config: AgentPluginConfig): Promise<AgentPlugin> {
    ...
    if (cached) {
      ...
      await cached.dispose().catch((err) => {
        console.error(`[AgentRegistry] Error disposing stale instance for ${cacheKey}:`, err);
      });
  ...
  private async initializeInstance(config: AgentPluginConfig, cacheKey: string): Promise<AgentPlugin> {
