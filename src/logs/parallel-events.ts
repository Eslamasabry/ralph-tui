...
const LOGGABLE_TYPES = new Set<ParallelEvent['type']>([
  ...
  'parallel:impact-missing',
  'parallel:impact-drift',
  'parallel:validation-queued',
  'parallel:validation-started',
  'parallel:validation-check-finished',
  'parallel:validation-passed',
  'parallel:validation-failed',
  'parallel:validation-fix-succeeded',
  'parallel:validation-reverted',
  'parallel:validation-blocked',
...
export async function appendParallelEvent(cwd: string, event: ParallelEvent): Promise<void> {
  ...
  for (const target of targets) {
    try {
      ...
      console.error(`[logs/parallel-events] Failed to write event to ${target}: ${error instanceof Error ? error.message : 'unknown error'}`);
    }
  }
}
